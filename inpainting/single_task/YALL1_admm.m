...
for i = 1 : max_iter
	...
	%v-step
	tv = A*x-y-w/rho;
    %v = repmat(prox_Lq_m(tv,1,lamda*rho),[1,L]) .* tv;  % multitask
    v = [prox_Lq(tv(:,1),1,lamda*rho), prox_Lq(tv(:,2),1,lamda*rho), prox_Lq(tv(:,3),1,lamda*rho)]; % single-task
        
    %x-step
    tao = 0.99; %for orthonornal A
    z = x - tao*(A'*(A*x - y - v - w/rho)); 
    %x = repmat(prox_Lq_m(z,1,rho/tao),[1,L]) .* z; % multitask
    x = [prox_Lq(z(:,1),1,rho/tao), prox_Lq(z(:,2),1,rho/tao), prox_Lq(z(:,3),1,rho/tao)]; % single-task
